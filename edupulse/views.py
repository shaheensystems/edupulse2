...
from utils.function.helperGetAtRiskStudent import (
    get_no_of_at_risk_students_by_program_offerings,
    get_no_of_at_risk_students_by_course_offerings,
)

from utils.function.helperGetTotalNoOfStudents import (
    get_total_no_of_student_by_program_offerings,
    get_total_unique_no_of_student_by_program_offerings,
    get_total_no_of_student_by_course_offerings,
    get_total_unique_no_of_student_by_course_offerings,
)

from utils.function.helperGetChartData import (
    get_chart_data_program_offerings_student_enrollment,
    get_chart_data_course_offerings_student_enrollment,
    get_chart_data_student_and_Staff_by_campus,
    get_chart_data_student_enrollment_by_region,
    get_chart_data_programs_student_enrollment,
    get_chart_data_offering_type_student_enrollment,
    get_chart_data_attendance_report,
)

from utils.function.helperDatabaseFilter import (
    filter_database_based_on_current_user,
    get_online_offline_program,
    default_start_and_end_date,
    filter_data_based_on_date_range,
)
...
class DashboardTestView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_data = filter_database_based_on_current_user(
            request_user=self.request.user
        )

        program_offerings_for_current_user = user_data[
            "program_offerings_for_current_user"
        ]
        course_offerings_for_current_user = user_data[
        ...
        context["pl_program_wise_attendance_detail_data"] = (
            pl_program_wise_attendance_detail_data
        )
        context["pl_campus_wise_attendance_detail_data"] = (
            pl_campus_wise_attendance_detail_data
        )
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        online_and_offline_programs = get_online_offline_program(
            programs_for_current_user=programs_for_current_user
        )
        ...
        context["chart_data_campus_enrollment_student"] = (
            chart_data_student_enrollment_by_campus
        )
        context["chart_data_campus_enrollment_staff"] = (
            chart_data_staff_enrollment_by_campus
        )
        context["chart_data_offering_mode_enrollment_students"] = (
            get_chart_data_offering_type_student_enrollment(
                course_offerings=course_offerings_for_current_user
            )
        )
        context["chart_data_programs_student_enrollment"] = (
            get_chart_data_programs_student_enrollment(programs=all_programs)
        )
        # context['chart_data_programs_student_enrollment'] = get_chart_data_programs_student_enrollment(programs=context['all_programs'])
        context["chart_data_program_offering_student_enrollment"] = (
            get_chart_data_program_offerings_student_enrollment(
                program_offerings=program_offerings_for_current_user
            )
        )
        context["chart_data_course_offering_student_enrollment"] = (
            get_chart_data_course_offerings_student_enrollment(
                course_offerings=course_offerings_for_current_user
            )
        )
        context["chart_data_student_region"] = (
            get_chart_data_student_enrollment_by_region(students=students)
        )
        ...
        ) = get_chart_data_attendance_report(attendances=attendances)

        context["chart_data_attendance_report_attendance"] = (
            chart_data_attendance_report_attendance
        )
        context["chart_data_attendance_report_engagement"] = (
            chart_data_attendance_report_engagement
        )
        context["chart_data_attendance_report_action"] = (
            chart_data_attendance_report_action
        )
        ...
        context["program_offerings_for_current_user"] = (
            program_offerings_for_current_user
        )
        ...
        if (
            ...
            context["total_students_in_program_offerings_for_current_user"] = len(
                get_total_unique_no_of_student_by_program_offerings(
                    program_offerings=program_offerings_for_current_user
                )
            )
        ...
        if (
            ...
            context["total_students_in_course_offerings_for_current_user"] = len(
                get_total_unique_no_of_student_by_course_offerings(
                    course_offerings=course_offerings_for_current_user
                )
            )
        ...
        blended_query = Q(
            student_enrollments__course_offering__offering_mode="micro cred"
        ) | Q(student_enrollments__course_offering__offering_mode="blended")
        if students:
            blended_students = students.filter(blended_query)
            blended_students = students.exclude(
                Q(student_enrollments__course_offering__offering_mode="online")
            )
            online_students = students.filter(
                Q(student_enrollments__course_offering__offering_mode="online")
            )
        else:
        ...
        if blended_students:
            context["total_students_in_blended_course_offerings"] = set(
                blended_students
            )
        else:
            ...
            for co in course_offerings_for_current_user:
                for r in co.student_enrollments.all():
                    ...
                    if co.offering_mode == "online":
                        enrolled_students_for_online_course_offerings.append(
                            new_student
                        )
                    else:
                        enrolled_students_for_blended_course_offerings.append(
                            new_student
                        )
        ...
        context["total_enrollment_in_blended_course_offerings"] = (
            enrolled_students_for_blended_course_offerings
        )
        context["total_enrollment_in_online_course_offerings"] = (
            enrolled_students_for_online_course_offerings
        )
        context["total_students_at_risk_query_set"] = (
            get_no_of_at_risk_students_by_program_offerings(
                program_offerings_for_current_user
            )
        )
        context["attendances"] = attendances
        context["student_attendance_percentage"] = (
            get_students_attendance_report_by_students(students=students)
        )
        context["staff_profile"] = (
            self.request.user.staff_profile
    ...
    def handle_date_filter_request(self, request, *args, **kwargs):
        ...
        if "start_date" in request.GET or "end_date" in request.GET:
            ...
            program_offerings_for_current_user = context[
                "program_offerings_for_current_user"
            ]
            course_offerings_for_current_user = context[
                "course_offerings_for_current_user"
            ]
            ...
            print(
                f" send campuses for date filter by get method from dashboard page :{campuses}"
            )
            if date_filter_form.is_valid():
                ...
                program_offerings_for_current_user = filtered_data_by_date_range[
                    "program_offerings_for_current_user"
                ]
                course_offerings_for_current_user = filtered_data_by_date_range[
                    "course_offerings_for_current_user"
                ]
                programs_for_current_user = filtered_data_by_date_range[
                    "programs_for_current_user"
                ]
                courses_for_current_user = filtered_data_by_date_range[
                    "courses_for_current_user"
                ]
                active_programs_for_current_user = filtered_data_by_date_range[
                    "active_programs_for_current_user"
                ]
                inactive_programs_for_current_user = filtered_data_by_date_range[
                    "inactive_programs_for_current_user"
                ]
    ...
    def handle_ajax_request(self, request, *args, **kwargs):
        ...
        if program_id or week_no or session_no:
            print(
                f"initializing attendance , engagement , acton chart data data  with program id :{program_id}, week no :{week_no} and session no :{session_no}"
            )

            if program_id:
                attendances = attendances.filter(
                    course_offering__student_enrollments__program_offering__program_id=program_id
                )

            if week_no:
                print(f"Week filter initiated .... for week no :{week_no}")
                attendances = attendances.filter(
                    Q(week_no=week_no) | Q(week_no=int(week_no) + 9)
                )
            ...
            html_content = render_to_string(
                "components/dashboard/student_attendance_engagement_action_report.html",
                {
                    ...
                    "programs_for_current_user": context[
                        "active_programs_for_current_user"
                    ],
                },
            )
        ...
        if pl_data_table_button_id:
            print(
                "initializing student count table  data  for student count by button clicked id:",
                pl_data_table_button_id,
            )
            ...
            if pl_data_table_button_id == "id_data_table_button_campus":
                ...
                table_button_name = "Program"
                programs_for_current_user = context.get(
                    "active_programs_for_current_user"
                )
                table_data = get_table_data_student_and_enrollment_count_by_programs(
                    programs=programs_for_current_user
        ...
        if pl_student_count_barChart_id:
            table_button_name = student_count_table_title
            print(
                "initializing bar Chart data  for student count by button clicked id:",
                pl_student_count_barChart_id,
            )
        ...
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            ...
            if course_offering_id:
                course_offering = get_object_or_404(
                    CourseOffering, temp_id=course_offering_id
                )
                students = course_offering.student.all()
                # week_numbers = course_offering.get_week_numbers()
                week_numbers = [
                    str(week_number[1])
                    for week_number in course_offering.get_week_numbers()
                ]
                ...
                return JsonResponse(
                    {"error": "Course offering ID is required"}, status=400
                )
        else:
            # Non-AJAX request
            ...
            default_course_offering_id = (
                context["course_offerings"].first().temp_id
                if context["course_offerings"]
                else None
            )
            default_week_number = "1"  # Set your default week number here
            default_student_id = (
                context["course_offerings"].first().student.first().temp_id
                if context["course_offerings"]
                and context["course_offerings"].first().student.first()
                else None
            )
    ...
    def post(self, request, *args, **kwargs):
        if request.headers.get("x-requested-with") == "XMLHttpRequest":
            ...
            if course_offering_id and week_number:
                ...
                start_date = course_offering.start_date + timedelta(
                    weeks=int(week_number) - 1
                )
                ...
                weekly_report_data = (
                    course_offering.weekly_reports.all()
                )  # Implement your logic here

                weekly_report_data = weekly_report_data.filter(
                    Q(sessions__attendance_date__gte=start_date)
                    & Q(sessions__attendance_date__lte=end_date)
                )
                ...
                # Render HTML using a Django template
                html_content = render_to_string(
                    "components/weekly_reports/weekly_reports_list.html",
                    {
                ...
                return JsonResponse(
                    {
                        "error": "Course offering ID, week number, and student ID are required"
                    },
                    status=400,
                )
        else:
            return JsonResponse(
                {"error": "This endpoint accepts only AJAX requests"}, status=400
            )
...
class SaveWeeklyReportsView(View):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(
                {"success": True, "message": "Weekly reports saved successfully"}
            )
